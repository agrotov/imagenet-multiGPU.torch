...
function train_imagenet_bandit(model)
   ...
   for t = 1,logged_data:size(1),opt.batchSize do
      ...
      for i = t,math.min(t+opt.batchSize-1,logged_data:size(1)) do
         ...
         -- load new sample
         print("class")
         print((index_of_input%1000)+1)
         print(math.floor(index_of_input/1000))
         local input, index_tmp = trainLoader:getByClassAndIndex((index_of_input%1000)+1, math.floor(index_of_input/1000))
         inputs[k] = input
         actions[k] = action
         rewards[k] = reward
