...
function probabilities_from_output(model_output, temperature)
    ...
    print("probabilities",probabilities,torch.sum(probabilities))
    print("exp probabilities",torch.exp(probabilities),torch.sum(torch.exp(probabilities)))
...
function trainBatch_bandit_fake(inputsCPU, actions_cpu, rewards_cpu, probabilities_logged_cpu, optimState, labelsCPU, temperature, batchNumber, baseline)
    model:evaluate()
...
--    if batchNumber % 10 == 0 then
        full_information_test(inputsCPU, labelsCPU, batchNumber, rewards_cpu, probabilities_logged, p_of_actions_student)
--    end
    dataTimer:reset()
